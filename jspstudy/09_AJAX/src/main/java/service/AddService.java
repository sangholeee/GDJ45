...
public class AddService implements MemberService {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException{
		...
		// insertMember() 메소드의 실행이 실패하는 경우(아이디 중복, 데이터 누락, 칼럼의 크기 오류 등)
		// Mybatis의 경우 PersistenceException 발생
		...
		try {
			...
			// 성공 했을 때 응답은 jQuery.jQuery.ajax의 success 프로퍼티로 넘어간다.
			out = response.getWriter();
			out.write(responseText);   // out.print(responseText), out.println(responseText) 모두 가능
									   // responseText가 manage.jsp의 ajax의 success로 간다.
			out.flush();
			out.close();
