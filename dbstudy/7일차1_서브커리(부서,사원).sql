...
--    서브쿼리 : 부서번호(DEPART)가 2인 부서에 근무하는 사원들의 직급(다중 행 : 부서번호(DEPART) 칼럼은 PK도 아니고 UNIQURE도 아니므로 서브쿼리 결과가 여러개)
...
--    서브쿼리 : 부서명(DEPT_NAME)이 '영업부'인 부서번호(다중 행 : 부서명(DEPT_NAME) 칼럼은 PK도 아니도 UNIQUE도 아니다.
...
--     부서번호(DEPART)가 1인 부서에 근무하는 모든 사원들의 급여와 비교해서 더 많이 받으면 조회하시오.
