...
--    EMPLOYEE 테이블에는 부서번호가 3인 행(ROW, RECORD)이 없기 때문
DELETE
    FROM DEPARTMENT
    WHERE DEPT_NO = 3;
    
-- 2) 부서번호가 1인 부서를 삭제하시오. (실패)
--    EMPLOYEE 테이블에는 DEPARTMENT 테이블의 부서번호 1을 참조하고 있는 행(ROW, RECORD)이 있기 때문
/*
ALTER TABLE EMPLOYEE
    ADD CONSTRAINT EMPLOYEE_DEPARTMENT_FK FOREIN KEY(DEPART)
        REFERENCES DEPARTMENT(DEPT_NO);
*/
