...
-- 메인쿼리에서 BOARD_NO, TITLE, CONTENT, MEMBER_ID, CREATE_DATE 칼럼만 사용할 수 있다.
...
SELECT MEMBER_NO, MEMBER_ID
  FROM MEMBERS
 WHERE ROWID = 'AAAFAHAABAAALD5AAA';   -- 실제로 사용할 수 없는 방식(대신 INDEX를 사용함)
...
-- 1. BOARDS 테이블에서 3번째로 작성한 게시글을 조회하시오.
...
-- 2. BOARDS 테이블에서 2~4번째로 작성한 게시글을 조회하시오.
...
--    작성일자 오름차순 정렬 이후 ROWNUM이 3인 데이터
...
-- 행번호를 먼저 붙인다음 정렬하기 때문에 정렬의 순서가 어긋난다.
SELECT A.RN, A.BOARD_NO, A.TITLE, A.CONTENT, A.MEMBER_ID, A.CREATE_DATE
  FROM (SELECT ROWNUM AS RN, BOARD_NO, TITLE, CONTENT, MEMBER_ID, CREATE_DATE
          FROM BOARDS
...
-- 3) RANK() OVER(ORDER BY 칼럼) : 동일한 순위(행번호)
