...
public class Ex01_String {
	...
	public static void main(String[] args) {
		...
		// 문제. "abc123def" 문자열에서 아라비아 숫자 "123" 제외하고 출력하기.
		...
		//    2) sbustring(begin, end) : 인덱스 begin(포함)부터 인덱스 end(불포함) 이전까지 추출
		...
		System.out.println(idx2);      // 1  (발견된 첫 번째 인덱스 반환)
		System.out.println(idx3);      // -1 (없는 경우)
				
		int idx4 = str6.lastIndexOf("j");		
		int idx5 = str6.lastIndexOf("a");		
		int idx6 = str6.lastIndexOf("z");		
		
		System.out.println(idx4);      // 0
		System.out.println(idx5);      // 3 (발견된 마지막 인덱스 반환)
		System.out.println(idx6);      // -1 (없는 경우)
		...
		for(int i = 0; i < files.length; i++) {
			if(files[i].endsWith("jpg") == false) {   // .jpg로 끝나면 true, 아니면 false
				System.out.println(files[i] + "파일을 첨부할 수 없습니다.");
			}
