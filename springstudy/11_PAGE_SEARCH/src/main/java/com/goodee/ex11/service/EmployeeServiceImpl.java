...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public void findEmployees(HttpServletRequest request, Model model) {
		...
		// 검색된 레코드 갯수 가져오기
		int findRecord = employeeMapper.selectFindCount(map);            // map : query, column, begin, end
		...
		List<Employee> employees = employeeMapper.selectFindList(map);   // map : query, column, begin, end, beginRecord, endRecord 
		...
		// /employee/list 이대로 넘기면 검색 결과나 나온 뒤 페이지 이동을 하면 검색이 풀리고 전체 리스트 보기가 된다.
		// /employee/search?column=FIRST_NAME&query=a&page=2
		//  path(findEmployees)->path.contains("?")  | PageUtils
