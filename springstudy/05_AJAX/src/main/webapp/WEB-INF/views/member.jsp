...
<script>
	...
	$(document).ready(function(){
		...
		$('#btn2').on('click', function(){
			fnAjax2();    // btn2을 클릭하면 fnAjax2() 함수를 실행하시오.
		})
		$('#btn3').on('click',
			fnAjax3       // btn3을 클릭하면 fnAjax3() 함수를 실행하시오.
		)
		$('#btn4').on('click',
			fnAjax4       // btn4을 클릭하면 fnAjax4() 함수를 실행하시오.
		)
	})
	...
	function fnAjax2(){
		...
		$.ajax({
			...
			success: function(obj){     // 응답 데이터는 obj로 전달
	...
	function fnAjax4(){
		...
		$.ajax({
			...
			// JSON 데이터를 문자열로 형식으로 만들어서 보냅니다.
			// webstudy/javascript/12_json
			data: JSON.stringify({
				'id': $('#id').val(),
				'pw': $('#pw').val()
			}),
