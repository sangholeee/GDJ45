...
public class BoardController {
	...
	@GetMapping("/")       
	public String index() {
		return "index";   // 뷰 리졸버에 의해서 "WEB-INF/views/index.jsp"로 변환 됩니다.
	}
	...
	/*
	 	...
	 	2. 필드, 생성자, setter에서 사용하는데 상황에 따라 생략
	 */
	...
	// @Autowired
	// private Board board1;         // root-context에서 Board를 찾아온 것. board1을 찾은 것이 아님.
	
	
	// 2. 생성자 주입
	//    생성자의 매개변수로 주입된다.
	//    @Autowired를 안 적어도 된다.
	//    lombok을 추가하고 @AllArgsconstructor를 사용하면 생성자가 생기기 때문에
	//    자동으로 주입되서 편함!
	
	// private Board board1;
	
	// public BoardController(Board board2) {     // 매개변수 Board를 보고 root-context에서 Board를 찾아 알아서 넣어준다. 여기도 board2를 보고 찾는거 아님!
	//	  super();
	//	  this.board1 = board2;
	// }
	...
	// 3. setter 주입
	//    setter의 매개변수로 주입된다.
	//    @Autowired를 setter 앞에 작성해준다.
	//    @Autowired가 추가된 메소드의 매개변수는 모두 주입된다. (꼭 setter가 아니여도 된다.)
	//    setBoard1 -> 아무거나 변경 가능
	...
	@Autowired
	@Qualifier(value="board1")   // <qualifier value="board1"/> 이 bean을 주입해 주시오.
	private Board board1;
	...
	@GetMapping("board/detail")   // 여기 "board/detail" 은 요청
	public String detail(HttpServletRequest request) {
		...
		return "board/detail";    // 뷰 리졸버에 의해서 return "/WEB-INF/views/board/detail.jsp" 변환됩니다.
								  // 여기 "board/detail" 은 응답 -- 두 개는 아예 다른거임!
	}
}
